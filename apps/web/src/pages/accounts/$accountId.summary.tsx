...
import { useAccountOverview, useAccount } from '../../api/queries/accounts';
...
export default function AccountSummaryPage() {
  ...
  const { data: overviewData, isLoading, error } = useAccountOverview(accountId);
  const sections = useMemo(
    () => (overviewData?.overview ? parseSummary(overviewData.overview) : []),
    [overviewData],
  );
      ...
      <Card title="Account Summary">
        {error ? (
          <div className="flex items-center gap-2 rounded-lg border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 px-4 py-3">
            <span className="text-sm text-red-600 dark:text-red-400">Failed to load account overview.</span>
          </div>
        ) : sections.length === 0 ? (
